...
module.exports = {
  ...
  async showCanvas(req, res) {
    try {
      ...
      // Ensure user belongs to the team that owns this canvas
      if (canvas.team_id && req.session.userId) {
        const membership = await teamModel.userInTeam(
          req.session.userId,
          canvas.team_id
        );
        if (!membership) {
  ...
  // ==============================
  // Save current canvas as artwork
  // (stores image_url + canvas_id + state_json)
  // ==============================
  async saveCanvas(req, res) {
    try {
      const { title, description, imageUrl, canvasId } = req.body;
      ...
      const cid = canvasId ? parseInt(canvasId, 10) : null;

      // ✅ capture the current persisted canvas JSON state
      const stateArray = cid ? await canvasModel.getState(cid) : [];
      ...
      res.status(500).json({
        success: false,
        message: 'Server error saving artwork'
      });
    }
  }
};
